<template>
	<view class="content">
		...
		<swiper :style="coHeight" :duration="500" :current="thisindex" :data-index='thisindex' @change="toggle" circular>
			<swiper-item>
...
<script>
	export default {
		...
		computed: {
			coHeight: function() {
				// 动态大小			// 20是margin-bottom	// 再加一个tabBar高度
				let height = (150 + 20) * this.transports.length + 35 + "rpx";
				console.log(height);
		...
		onShow() {
			uni.request({
				url: 'http://hkkiack8.xiaomy.net:32771/tripapp/transports/',
				data: '',
				method: 'GET',
				success: (res) => {
		...
		methods: {
			...
			toggle(e) {
				...
				console.log(index)
				this.thisindex = this.target = index
				this.getTransportsList();
			},
			// 点击nav控制下面的展示
			setIndex(e) {
			...
			getTransportsList: function() {
				if (this.thisindex == 0) {
					...
					uni.request({
						url: 'http://hkkiack8.xiaomy.net:32771/tripapp/transports/train',
						data: '',
						method: 'GET',
						success: (res) => {
							this.transports = res.data.data;
					...
					uni.request({
						url: 'http://hkkiack8.xiaomy.net:32771/tripapp/transports/plane',
						data: '',
						method: 'GET',
						success: (res) => {
							this.transports = res.data.data;
					...
					uni.request({
						url: 'http://hkkiack8.xiaomy.net:32771/tripapp/transports/boat',
						data: {},
						method: 'GET',
						success: (res) => {
							this.transports = res.data.data;
